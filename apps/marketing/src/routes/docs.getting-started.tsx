...
function GettingStartedPage() {
  return <div className="marketing-start-layout">
      ...
      <section className="marketing-start-hero" aria-labelledby="getting-started-title">
        ...
        <aside className="marketing-start-aside"><strong>Before you begin</strong><p>You will need a deployment profile, a configured registry environment, a source if you plan to pull through, and a selected agent scope such as Codex or Claude. Use the configured sign-in when your pilot environment is ready.</p></aside>
      </section>
      ...
      <div className="marketing-start-content">
        <div className="marketing-start-steps">
          ...
          <section className="marketing-start-step" id="review"><span className="marketing-start-step-number">04</span><div className="marketing-start-step-copy"><h2>Review the release decision</h2><p>Look for the release identity, source revision, artifact digest, policy revision, and scanner evidence. A release is available only when the configured policy admits it.</p><div className="marketing-start-code"><pre><code>pskills scan status sha256:&lt;release-digest&gt;{ '\n' }pskills pack show @team/frontend</code></pre></div></div></section>
          ...
          <section className="marketing-start-step" id="rollout"><span className="marketing-start-step-number">07</span><div className="marketing-start-step-copy"><h2>Plan the next team</h2><p>When the first workflow is clear, decide which teams need source access, which scanner modes are required, and whether fixed packs should become the shared interface. When your pilot environment is ready, use its configured sign-in and continue from the registry onboarding view.</p><a className="marketing-button marketing-button-primary" href={appLoginHref()}>Sign in to registry <span aria-hidden="true">↗</span></a></div></section>
        </div>
