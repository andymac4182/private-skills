...
function DocsPage() {
  return <div className="marketing-docs-layout">
    <article className="marketing-docs-main">
      ...
      <p>Start with the short demo, then follow the smallest useful path from an application workspace or local operator setup to a repeatable skill install. The pages also explain product boundaries, so an engineering team can evaluate the workflow before a larger rollout.</p>
      <div className="marketing-docs-cards">
        ...
        <a className="marketing-doc-card" href="/docs/getting-started"><span className="marketing-doc-card-index">02 / START HERE</span><h2>Getting started</h2><p>Choose an application workspace or local operator setup, set a policy, bring in one source, and install one approved release with the CLI.</p><span>Open the guide ↗</span></a>
