...
import type { Policy, ScanResult, SkillVersion } from '../lib/types'
...
function SkillRow({ skill, needsRescan: releaseNeedsRescan, selected, onSelect }: { skill: SkillVersion; needsRescan: boolean; selected: boolean; onSelect: () => void }) {
  return <tr className={selected ? 'row-selected' : ''}><td><button className="link-button" type="button" onClick={onSelect}><strong>{skill.name}</strong><span className="cell-sub">{skill.description || 'No description'}</span></button></td><td>{skill.version}</td><td><Badge tone={releaseNeedsRescan ? 'warn' : undefined} value={releaseNeedsRescan ? 'needs rescan' : skill.state} /></td><td>{skill.fileCount}<span className="cell-sub">{formatBytes(skill.artifact.size)}</span></td><td>{formatDate(skill.createdAt)}</td><td><button className="row-link" type="button" onClick={onSelect}>Inspect</button></td></tr>
}
