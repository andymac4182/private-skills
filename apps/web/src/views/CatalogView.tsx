...
function canAuthorDraft(principal: Principal | null | undefined): boolean {
  if (!principal || !principal.roles.some((role) => role === 'owner' || role === 'admin' || role === 'publisher')) return false
  if (!Array.isArray(principal.scopes)) return true
  return principal.scopes.some((scope) => scope === '*' || scope === 'registry:*' || scope === 'skills:write' || scope === 'skills:publish')
}
