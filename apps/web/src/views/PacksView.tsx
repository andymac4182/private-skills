...
export function PacksView() {
  ...
  return <div className="view-heading"><div><span className="eyebrow">Curated releases</span><h1>Packs</h1><p className="muted">Create approved collections that install consistently across projects and machines.</p></div><div className="grid-2"><Panel title="Create a pack" description="Members are selected and fixed when you create the pack."><form className="form-grid" onSubmit={create}><Field label="Pack name"><input onChange={(event) => setName(event.target.value)} placeholder="@team/web" value={name} /></Field><Field label="Version"><input onChange={(event) => setVersion(event.target.value)} placeholder="1.0.0" value={version} /></Field><Field label="Description"><textarea onChange={(event) => setDescription(event.target.value)} placeholder="Approved web engineering skills" value={description} /></Field><Field label="Members" hint="One release per line. Fixed versions keep the collection consistent."><textarea onChange={(event) => setMembers(event.target.value)} placeholder="@team/review@1.2.0\n@team/accessibility@2.0.1" value={members} /></Field>{message && <div className="full"><Notice kind={message.kind}>{message.text}</Notice></div>}<div className="form-actions"><Button busy={busy} type="submit">Create pack</Button></div></form></Panel><Panel title="Published packs" description="Published collections and the skills they include.">{error ? <ErrorState message={error} onRetry={() => void load()} /> : packs === null ? <LoadingState /> : packs.length === 0 ? <EmptyState title="No packs yet" description="Create the first curated collection once its member skills are approved." action={<Link className="button button-secondary" to="/app/$section" params={{ section: 'catalog' }}>Review catalog</Link>} /> : <div className="table-wrap"><table><thead><tr><th>Pack</th><th>Members</th><th>State</th><th>Published</th></tr></thead><tbody>{packs.map((pack) => <tr key={pack.id}><td><strong>{pack.name}</strong><span className="cell-sub">{pack.version} · {shortDigest(pack.manifestDigest)}</span></td><td>{pack.members.length}</td><td><span className="badge badge-good">{pack.state}</span></td><td>{formatDate(pack.createdAt)}</td></tr>)}</tbody></table></div>}</Panel></div></div>
}
