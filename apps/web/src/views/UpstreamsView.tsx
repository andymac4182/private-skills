...
export function UpstreamsView() {
  ...
  const sessionRole = session?.activeMembership?.role
  const canManageSources = sessionRole !== undefined
    ? sessionRole === 'owner' || sessionRole === 'admin'
    : principal?.roles.some((candidate) => candidate === 'owner' || candidate === 'admin') ?? false
  const canRequestImports = canManageSources || (sessionRole === 'publisher' || (sessionRole === undefined && principal?.roles.includes('publisher') === true))
