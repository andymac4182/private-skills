...
tests passed, and 44 skipped. Typecheck, the Node Nitro build, the Vercel Build
Output profile, and the Cloudflare edge build passed. Separate disposable
...
from `a74dc3e`, the exact cherry-pick of `968363c7` included in PR #60,
documents the deployed corrected access/preview copy. PR #59 delivered the
...
| --- | ---: | --- |
| Product/public surface | 4 | Hosted identity/onboarding/SSO; the integrated two-company browser journey with required-scan and worker/Eve audiences; hosted publish/review/install with source/search/analytics isolation; final public pricing and cross-site review. The populated local B05/L10 matrix is recorded as complete for its bounded scope. |
...
| --- | --- | --- | --- |
| Product + Engineering | **Positioning:** the product is a private registry and governed pull-through for versioned agent skills and packs. The user journey is source or upload → validation → required scan → policy decision → immutable release → authorized CLI install. Homepage copy must say that scan evidence is a policy input, not a harmlessness or agent-behavior guarantee. | `README.md:1-3,53-68`, `docs/product.md`, `docs/business/launch-plan.md`, `apps/marketing/src/routes/index.tsx`, and `docs/evidence/production-release-marketing-claims-ee7d3c35-20260916.json`. | **Public slice verified:** PR #60's durable record verifies the separate marketing route, nine public routes, corrected access copy, and app readbacks at `ee7d3c`. Final hosted application journey and cross-site navigation review remain open. |
...
| Product + Support | **Onboarding:** a new user can see provider choices, sign in, choose or create a company, and use a copy-link invitation. An authenticated user with no membership receives an onboarding response and no registry data. Invitation delivery is not described as email until an email transport is configured and tested. | Integrated identity BFF (`/auth/identity/config`, `/auth/identity/session`), `apps/web/server/company-sso-runtime.ts`, `docs/identity.md`, `docs/identity-company-sso.md`, `docs/evidence/local-invitation-membership-acceptance-20260916.json`, and the local browser journey. | Better Auth identity BFF, onboarding source, company selector UI, invitation copy-link source, company SSO runtime/discovery/picker source, and local PostgreSQL composition are integrated. The local invitation record covers owner create, member acceptance, Reader role denial, revocation, and expiry in disposable fixtures. The latest [hosted schema-preparation record](../evidence/hosted-identity-schema-preparation-20260916-attempt-2.json) verifies 12 empty identity/SSO/token/operations tables, while production identity remains deliberately disabled (`providers: []`). Hosted browser flow and a real customer provider callback remain open; no email delivery is claimed. Manual link/code is the launch path; 16–18 Sep. |
...
| `docs/evidence/local-storage-billing-restoration-20260916.json` | Candidate validation from source `036fb74`: 178 test files passed/18 skipped, 1,281 tests passed/44 skipped, typecheck and Node/Vercel Build Output/Cloudflare builds, 34 direct PostgreSQL tests, and 2 composed Nitro tests. | The checks are local/disposable and do not close provider finality, hosted recovery, billing recovery, hosted bindings, or a final hosted customer flow. |
...
| `docs/evidence/marketing-claims-pricing-audit-20260916.md` | `a74dc3e` claims/pricing and preview-copy audit, an exact cherry-pick of `968363c7` included in PR #60, with nine-route rendering and explicit access/billing labels. | It does not prove a live provider, billing account, or commercial offer; PR #60 is the production readback for the released access copy. |
| `docs/evidence/marketing-usability-audit-20260916.md` | Historical local `9f46983` marketing follow-up with nine public routes, axe cleanliness, and manual gradient-contrast review. | It is local historical evidence; PR #59 is the production record for the deployed contrast change. |
...
- The [candidate validation record](../evidence/local-storage-billing-restoration-20260916.json)
  ...
  PostgreSQL tests, 2 composed Nitro tests, and passing typecheck, Node, Vercel
  Build Output, and edge builds. The [populated B05/L10 matrix](../evidence/local-b05-l10-populated-tenant-matrix-b7efd48-20260916.json)
...
- Resolve the Hobby/Fluid plan or adaptation choice, then record worker
  ...
  minimum.
- Complete the remaining public pricing and offer review against the server
  catalog, keeping preview and no-purchase language until the offer and Stripe
  gates are approved. PR #60 records the deployed `a74dc3e` corrected access
  copy, and PR #59 records the deployed contrast change. Native Linux is
  emulated and Windows remains unverified.
