...
- [ ] B26 — Hosted CLI archive delivery is partially verified. The authenticated company metadata/download route, platform chooser and pinned size/digest checks are integrated in `1b2671a`; production provisioning and HTTP readback now pass for all three v0.4.0 targets: the authenticated catalog reports every asset ready, authenticated downloads return 200 with `no-store` and exact manifest bytes/digests, and an anonymous download returns 401. See [production release evidence](../../evidence/production-release-11b5e1d.json) and the [hosted manifest](../../evidence/hosted-cli-v0.4.0-private-manifest-20260916.json). The remaining gate is an authenticated hosted browser chooser → download → install walkthrough. The macOS ARM64 native smoke check passed locally; Linux and Windows archive members/checksums are verified, but native Linux/Windows execution and native CI remain unverified and no native pass is claimed. Owner: oauth_demo_finish.
