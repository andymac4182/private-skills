...
- [ ] B26 — Hosted CLI archive acquisition remains open. The authenticated company metadata/download route, platform chooser and pinned size/digest checks are integrated in `1b2671a`; local verification covered the focused route/UI/package tests and TypeScript (`327b29e`/`31bdf18`). The [native Company B install proof](../../evidence/local-nitro-native-cli-install-20260916.json) and [local asset staging record](../../evidence/local-cli-v0.4.0-assets-staging-20260916.json) explicitly show local-only scope: the default manifest is unprovisioned, no Files SDK upload or public archive URL exists, and cross-company hosted acquisition is unverified. Provision production assets, authenticated hosted metadata/downloads and browser install verification before closing this gate. Owner: oauth_demo_finish.
