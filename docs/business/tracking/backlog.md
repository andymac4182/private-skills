...
- [ ] B27 — Explicit identity migration must preserve the location and validity of existing API tokens when a custom Better Auth schema is already configured. Require separate explicit token-schema configuration or a reviewed data-preserving migration; prove the existing custom-identity/public-token combination. Owner: editor_test_stability. Candidate 5ca2365 held pending correction.
