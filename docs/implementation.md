...
| Billing | `packages/billing/src`, `apps/web/server/routes/billing.ts`, `apps/web/server/runtime*.ts`, `apps/web/src/views/BillingView.tsx` | Provider-neutral organization customer/subscription mappings, finite provisional plan catalog, signed raw-body webhook processing, separate PostgreSQL billing tables, durable event deduplication, transactional usage reservations, a server-only Stripe invoice read model, an owner/admin company billing console, explicit durable providerless metering, and a browser-safe public plan metadata projection used by the separate marketing preview | The Node runtime mounts tenant billing routes and the top-level raw webhook handler, injects the verified invoice adapter, keeps payment actions closed for providerless metering, and keeps live billing disabled without a durable PostgreSQL boundary. The focused runtime, provider, route, UI, and marketing projection tests pass. Company-admin navigation selection remains a host UI integration seam. Stripe account credentials, provider Price IDs, production charges, and public launch prices remain deployment/product decisions. See [`billing.md`](billing.md) and [`../apps/marketing/README.md`](../apps/marketing/README.md) |
...
| Hosted worker | `workers/runner/src/hosted.ts`, `packages/scanners/src/sandbox-executor.ts`, `packages/database/src/hosted-worker-dispatch.ts`, `apps/web/server/runtime*.ts` | Authenticated one-shot Node/Vercel Sandbox execution plus bounded multi-company dispatch with a durable cursor, fenced lease, per-company retry backoff, and server-selected tenant credentials | The dispatcher requires Better Auth organizations, PostgreSQL, signed delegation, and explicit dispatch-table migration; edge builds do not run it. The Vercel build emits and verifies a function duration budget with persistence headroom; see [`vercel-function-budget.md`](vercel-function-budget.md). Production worker/scanner evidence remains deployment-specific |
...
The request boundary is `createRegistryHandler`, a host-neutral `Request` to `Response` function. `apps/web/server/runtime.ts` composes it with an environment-selected infrastructure factory and also exposes worker-only state, blob gateway, intelligence, hosted-worker, and company billing routes. Billing webhook requests are handled at the top-level raw-body boundary; authenticated tenant billing routes run before intelligence and registry dispatch. Successful publish, import, and rescan mutations schedule a bounded two-job hosted-worker drain through Nitro's `waitUntil` hook when the platform provides it. The Node profile in `runtime-node.ts` supports local file state, PostgreSQL state, HTTP state, Files SDK providers, HTTP blob storage, pgvector search, StateRepository search fallback, the optional hosted worker, and the server-only billing provider adapter. The edge profile in `runtime-edge.ts` requires authenticated HTTP state and blob endpoints so the edge request process does not require native filesystem, PostgreSQL, provider SDK, scanner execution, or live billing credentials.
...
| `GET /internal/worker/run` | CRON_SECRET-protected hosted scanner invocation; when the tenant dispatcher seams are configured, walks one bounded durable organization page and invokes server-selected tenant workers |
...
The storage boundary computes a SHA-256 digest over the canonical bytes. `FilesSdkBlobStore` allocates a fresh random `sealed/` key, persists that identity in a storage attempt before upload, reads the object back, verifies size and digest, and never uses a digest as a mutable provider key. A host may provide a stable, non-secret provider binding; after an awaited upload and exact read-back verification, core and authoring persist a matching `StorageWriteReceipt` with the orphan transition. A missing binding or any ambiguous provider outcome produces no receipt and remains subject to provider-finality recovery. `RecoverableBlobStore` and `StorageRecoveryService` retain ambiguous writes until a dedicated operator proof, exact object inspection, optional explicit cleanup, confirmed absence, and exact billing-zero reconciliation all succeed. The HTTP blob gateway transfers bytes with bounded bodies, origin checks, authorization, timeout handling, and digest/descriptor validation, including same-key recovery writes. Published metadata references the verified object and cannot be replaced in place through the registry API.
...
The Node Files SDK factory derives a provider binding from non-secret storage configuration and the private prefix when the host does not pass one explicitly. The exported `resolveNodeStorageProviderBinding` applies the same validation to hosts that compose `createNodeFilesClient` themselves. Vercel Blob, R2 without an account or endpoint, and Azure without an account or endpoint remain usable without a binding, but their writes cannot mint receipts until the host supplies one. Vercel Blob can derive an identity from its non-secret store ID; otherwise the web runtime should pass a bounded `PSKILLS_STORAGE_PROVIDER_BINDING` value for receipt recovery, or pass `PSKILLS_STORAGE_BLOB_STORE_ID` through the Files SDK credentials. Hosts that use another adapter or a custom provider must expose an equivalent stable binding before relying on verified receipt cleanup.
...
The core creates durable `scan` and `import` jobs with a captured policy revision, artifact identity, and lease metadata. `WorkerApiClient` claims through an authenticated internal route and sends a fencing token on artifact reads and completion. A stale lease cannot finalize evidence for a newer attempt. A worker completion may carry the authenticated `scanInvocationStarted` boundary and the billing `reservationGeneration` returned at admission; the core stores both the terminal metered settlement intent and exact lifecycle token in the same job transaction, and a bounded server maintenance pass reconciles definite pre-scanner failures only for that job and billing generation. Older workers omit the marker or generation and remain charged conservatively.
