...
export interface ReviewRun {
  ...
  eveSessionId?: string;
  /**
   * Bounded provenance captured by the reviewer service. `invocationId` is
   * generated by the Eve app and is not a provider cron request ID.
   */
...
export type ReviewRunProvenanceSource = 'eve-schedule' | 'api';

export interface ReviewRunProvenance {
  source: ReviewRunProvenanceSource;
  invocationId: string;
...
export interface BeginReviewRunInput {
  ...
  eveSessionId?: string;
  /** Sanitized trigger metadata captured by the Eve reviewer app. */
  provenance?: ReviewRunProvenance;
  now?: ReviewNow;
}
...
function normalizeBeginInput(input: BeginReviewRunInput): {
  ...
  const eveSessionId = input.eveSessionId === undefined
    ? undefined
    : boundedOpaqueId(input.eveSessionId, 'eveSessionId', MAX_ID_LENGTH);
  const provenance = input.provenance === undefined
    ? undefined
    : normalizeProvenance(input.provenance);
  if (provenance !== undefined && eveSessionId === undefined) {
    throw new ReviewValidationError('provenance requires an Eve session ID');
  }
  return {
...
function normalizeProvenance(value: unknown): ReviewRunProvenance {
  if (!isRecord(value)) throw new ReviewValidationError('provenance must be an object');
  const source = value.source;
  if (source !== 'eve-schedule' && source !== 'api') {
    throw new ReviewValidationError('provenance source is invalid');
  }
  const invocationId = boundedOpaqueId(value.invocationId, 'provenance.invocationId', MAX_ID_LENGTH);
  const observedAt = boundedString(value.observedAt, 'provenance.observedAt', 64);
  try {
    if (new Date(observedAt).toISOString() !== observedAt) throw new Error();
  } catch {
    throw new ReviewValidationError('provenance.observedAt must be a canonical ISO timestamp');
  }
  const scheduleId = value.scheduleId === undefined
    ? undefined
    : boundedOpaqueId(value.scheduleId, 'provenance.scheduleId', MAX_ID_LENGTH);
  if (source === 'eve-schedule' && scheduleId === undefined) {
    throw new ReviewValidationError('scheduled provenance requires a schedule ID');
  }
  if (source === 'api' && scheduleId !== undefined) {
    throw new ReviewValidationError('API provenance must not include a schedule ID');
  }
  return {
    source,
    invocationId,
  ...
  async beginRun(organizationId: string, input: BeginReviewRunInput): Promise<ReviewRunClaim> {
    ...
    return this.repository.transaction(organizationId, (rawState) => {
      ...
      if (existing) {
        ...
        if (normalized.provenance !== undefined) {
          // A failed or expired lease is a new claimant. Keep the provenance
          // joined to that claimant, without rebinding active/completed runs.
